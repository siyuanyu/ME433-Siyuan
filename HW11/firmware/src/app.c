...
void oledprint(int printcol, int printrow, char asciimessage[]);


unsigned char accels[6]; // accelerations for the 3 axes
short acceldata;
char message[50];
int cpostatus = 0;
int datastatus = 0;
            ...
            else if( appData.hidDataReceived )
            {
                ...
                switch(appData.receiveDataBuffer[0])
                {
                    case 0x0:
                        ...
                        display_clear();
                        oledprint(1,appData.receiveDataBuffer[1],appData.receiveDataBuffer);
                        display_draw();
                    ...
                    case 0x1:
                        ...
                        if(appData.hidDataTransmitted)
                        {
                            ...
                            if (_CP0_GET_COUNT() > 200000) {
                                ...
                                acc_read_register(OUT_X_L_A, accels, 6);
                                LATBINV = 0b10000000;
                                int jj;
                                short total;
                                short movingavg;

                                acceldata = accels[4] | (accels[5]<<8);
                                ...
                                appData.transmitDataBuffer[2] = accels[4];
                                appData.transmitDataBuffer[3] = accels[5];
                                appData.transmitDataBuffer[4] = movingavg;
                                appData.transmitDataBuffer[5] = movingavg>>8;
                                //appData.transmitDataBuffer[2] = (~(acceldata)+1);
                                //appData.transmitDataBuffer[3] = (~(acceldata)+1) >> 8;
                                _CP0_SET_COUNT(0);
                            }
                            else {
                                appData.transmitDataBuffer[0] = 1;
